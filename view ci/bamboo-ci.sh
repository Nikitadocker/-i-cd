...
git checkout $3 # переключаемся на ветку билда и проверяем значение кода
if [ $? != 0 ]; #  проверяем значение кода
  then git checkout develop && git checkout -b $3  > /dev/null 2>&1; # тогда создаем из develop ветку билда
fi
